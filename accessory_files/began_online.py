...
class Data(Dataset):
    def __init__(self):
        im_dir = 'bub_single_32/'

        self.data = torch.Tensor([cv2.imread(im_dir + im, cv2.IMREAD_GRAYSCALE) for im in os.listdir(im_dir)])
...
for epoch in range(opt.n_epochs):
    for i, imgs in enumerate(dataloader):
        ...
        real_imgs = real_imgs.view(real_imgs.shape[0], 1, real_imgs.shape[1], real_imgs.shape[2])
