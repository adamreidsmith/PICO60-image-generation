...
# Subtract scaled sum/norm of image from generator loss to encourage white spots (reduce convergence to blank image)
# Apply thresholding to generator image
# Make loss of generator a combination of loss from generated image and thresholded image
...
write_dir = '../images/began_mod_test/'
...
class Generator(nn.Module):
    ...
    def forward(self, noise):
        ...
        #generated_im = t.where(generated_im < 0.1, Zeros, generated_im)
        return generated_im
...
gamma = 0.9 # gamma in [0,1] controls image diversity.  Higher values of gamma lead to more diverse images
...
for epoch in range(n_epochs):
    for i, data_imgs in enumerate(dataloader):
        ...
        G_loss = Loss(gen_imgs) #- 0.35*t.sum(gen_imgs)/(batch_size * channels * img_size**2)  #Maybe use this and apply binarization to the images after?
        ...
        if batches_done % sample_interval == 0:
            ...
            for j, gen_im in enumerate(gen_imgs):
                ...
                for T in range(4,7):
                    T = float(T)
                    thres_im = t.where(gen_im[0] > T/20, t.ones_like(gen_im[0]), t.zeros_like(gen_im[0]))
                    cv2.imwrite('%s%d/%d_%f.png' % (write_dir, batches_done, j, T/20), thres_im.detach().numpy()*255)'''
