function [fitparams, cam1params, cam2params, cam3params, cam4params] = FitPICO60Geometry()
% Fit the PICO-60 geometry and return the fit parameters as well as an
% array of the camera pixels containing pixel coordinates for each camera.  
% This function is adapted from runthenewgeometryfitter_pico60_2016.m.
...
% res = [1700 1088];
% 
% Define an arrray of pixels for all cameras
% pixels = zeros(res(1)*res(2),2);
% 
% for i = 1:res(1)
%     for j = 1:res(2)
%         pixels(res(2)*(i-1)+j, :) = [i j];
%     end
% end
% px_cam1 = [ones(length(pixels), 1) pixels];
% px_cam2 = [2*ones(length(pixels), 1) pixels];
% px_cam3 = [3*ones(length(pixels), 1) pixels];
% px_cam4 = [4*ones(length(pixels), 1) pixels];
% 
% pixels = [px_cam1; px_cam2; px_cam3; px_cam4];
% 
% pixels = pixels(1:10000:length(pixels), :);
% 
% fprintf('Running EricsAutomaticGeometryGoodnessWithTorus_lsq.m for realz\n');
% tic;
% EricsAutomaticGeometryGoodnessWithTorus_lsq(fitparams, paramstruct.bestfit, whichparams, ...
%     pixels, pixels, pixels(:,1), 'float', 1);
% toc;

fitparams = paramstruct.bestfit;

cam1params = [fitparams(1:22) fitparams(89:end)];
cam2params = [fitparams(23:44) fitparams(89:end)];
cam3params = [fitparams(45:66) fitparams(89:end)];
cam4params = [fitparams(67:88) fitparams(89:end)];
